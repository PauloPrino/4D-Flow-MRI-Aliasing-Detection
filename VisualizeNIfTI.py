...
def visualize_nifti_slices(nii_gz_path, time_frame, position_x, position_y, position_z):
    ...
    position_x: position of the sagittal slice (value between 0 and 143, because there are 144 slices in each direction)
    """
    ...
    axial_view = axes[0].imshow(slice_x.T, cmap="gray", origin="lower")
    axes[0].set_title(f"Axial Slice at position z={position_z}") # the pixels on the image are velocities in the direction of z

    coronal_view = axes[1].imshow(slice_y.T, cmap="gray", origin="lower")
